...
# Data for deep gravity ---------------------------------------------------

cz_features |>
  dplyr::select(-c(cz_name, state)) |>
  readr::write_csv(here::here("scratch/commuting_zone_features.csv"))

opp_migration |>
  dplyr::select(
    -c(o_cz_name, o_state_name, d_cz_name, d_state_name, pool, pr_d_o, pr_o_d)
  ) |>
  dplyr::rename("flow" = "n", "o_pop" = "n_tot_o", "d_pop" = "n_tot_d") |>
  dplyr::arrange(o_cz, d_cz) |>
  dplyr::left_join(
    cz_centroids[, c("commuting_zone_id", "geometry")],
    by = c("o_cz" = "commuting_zone_id")
  ) |>
  dplyr::rename("o_geometry" = "geometry") |>
  dplyr::left_join(
    cz_centroids[, c("commuting_zone_id", "geometry")],
    by = c("d_cz" = "commuting_zone_id")
  ) |>
  dplyr::rename("d_geometry" = "geometry") |>
  dplyr::mutate(
    o_d_distance = sf::st_distance(o_geometry, d_geometry, by_element = TRUE),
    o_d_distance = as.numeric(o_d_distance/1000)
  ) |>
  dplyr::select(-dplyr::contains("geometry")) |>
  readr::write_csv(here::here("scratch/commuting_zone_flows.csv"))

# Other stuff for full panel ----------------------------------------------
...
opp_migration_ns_modeling <- opp_migration_ns |>
  ...
  dplyr::select(-c(o_census_2000_population, d_census_2000_population))
