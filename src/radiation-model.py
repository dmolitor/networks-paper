...
def rmse(data, truth, estimate, digits=5):
    rmse_col = data[estimate] - data[truth]
    rmse_col = rmse_col.apply(lambda x: x**2)
    rmse = rmse_col.mean()**(1/2)
    return rmse
...
cz_gravity_cpc = (
    ...
    .apply(
        lambda x: {
            "gravity_cpc":cpc(x, "min_g_r_flow", "sum_g_r_flow"),
            "gravity_rmse":rmse(x, "flow", "gravity_flow")
        }
    )
    .reset_index()
    .rename(columns={0:"gravity_measures"})
)
cz_gravity_cpc["gravity_cpc"] = (
    cz_gravity_cpc["gravity_measures"]
    .apply(lambda x: x["gravity_cpc"])
)
cz_gravity_cpc["gravity_rmse"] = (
    cz_gravity_cpc["gravity_measures"]
    .apply(lambda x: x["gravity_rmse"])
)
cz_gravity_cpc["origin"] = cz_gravity_cpc["origin"].astype(int)
cz_gravity_cpc.drop("gravity_measures", axis=1, inplace=True)
...
cz_rad_cpc = (
    ...
    .apply(
        lambda x: {
            "rad_cpc":cpc(x, "min_g_r_flow", "sum_g_r_flow"),
            "rad_rmse":rmse(x, "flow", "radius_flow")
        }
    )
    .reset_index()
    .rename(columns={0:"rad_measures"})
)
cz_rad_cpc["rad_cpc"] = (
    cz_rad_cpc["rad_measures"]
    .apply(lambda x: x["rad_cpc"])
)
cz_rad_cpc["rad_rmse"] = (
    cz_rad_cpc["rad_measures"]
    .apply(lambda x: x["rad_rmse"])
)
cz_rad_cpc["origin"] = cz_rad_cpc["origin"].astype(int)
cz_rad_cpc.drop("rad_measures", axis=1, inplace=True)
...
cz_out = cz[[
    ...
    "tot_outflow", "gravity_cpc", "rad_cpc", "gravity_rmse", "rad_rmse"
]]
